...
const SOLANA_TRACKER_API_KEY =
  process.env.API_KEY || "f6b7c975-ec98-4160-99b9-9014ea88de2a"; // <-- Update your aggregator key
const DB_PATH = "bot_database.db";
const DEFAULT_SLIPPAGE = 1;
const BOT_TOKEN = "8040728733:AAHvlsRrxm_Z0GZhF4wMNzt0NHvpFk3NA-0"; // Hard-coded as in original
const HELIUS_API_KEY =
  process.env.HELIUS_API_KEY || "bc4dcb84-187c-4f7e-8503-cd42a6a10c6b";
...
function updateMainMenuMessageId(chatId, messageId) {
  if (!userSessions[chatId]) {
    userSessions[chatId] = {};
  }
  userSessions[chatId].mainMenuMessageId = messageId;
  logger.info(`Updated main menu message ID for ${chatId}: ${messageId}`);
}
...
async function getAllTokenBalances(pubkeyStr) {
  try {
    const c = new Connection(SOLANA_RPC_URL, "confirmed");
    const t = await c.getParsedTokenAccountsByOwner(new PublicKey(pubkeyStr), {
...
async function showMainMenu(chatId, messageId) {
  try {
    ...
    if (!u || !u.public_key) {
      ...
      message = `🚀 *Welcome to Beer Trading Bot!*\n\n`;
      message += `The future of Trading is at your fingertips!\n\n`;
      message += `🔐 *Address*\n(No wallet linked)\n\n`;
      message += `💰 *Balance*\n`;
...
replyMarkup = {
    ...
    updateMainMenuMessageId(chatId, messageId);
...
bot.onText(/\/start/, async (msg) => {
  try {
    ...
    const loadingMsg = await bot.sendMessage(chatId, `🚀 Loading Beerbot...`, {
      parse_mode: "Markdown",
    });

    await showMainMenu(chatId, loadingMsg.message_id);
    // The main menu ID is already tracked in showMainMenu
  } catch (err) {
  ...
  try {
    ...
    switch (d) {
        ...
        await bot.answerCallbackQuery(query.id);
        {
          const pm = await bot.sendMessage(
            ...
            "Please enter your private key to verify your wallet.",
            {
              reply_markup: {
                inline_keyboard: [
                  [{ text: "« Cancel", callback_data: "BACK_MAIN_DELETE" }],
                ],
              },
            },
          );
      ...
      case "BACK_MAIN_DELETE":
        await bot.answerCallbackQuery(query.id);
        {
          // Get the main menu message ID
          const mainMenuId = userSessions[c]?.mainMenuMessageId;

          // If we have a valid main menu ID
          if (mainMenuId && mainMenuId !== mid) {
            try {
              // Delete the current message
              await bot.deleteMessage(c, mid);

              // Refresh the main menu
              await showMainMenu(c, mainMenuId);
            } catch (e) {
              logger.error("Error handling BACK_MAIN_DELETE:", e);
              // If deleting fails, just go back to main menu as normal
              await showMainMenu(c, mid);
            }
          } else {
            // If no main menu ID or it's the same as current, just show main menu
            await showMainMenu(c, mid);
          }
        }
        break;
...
`;
      ...
      case "AUTO_TRADE":
        await bot.answerCallbackQuery(query.id);
        {
          ...
          const userBalMsg = `🚀 *Auto-Trade Activation*  
...
${wBalanceLine}`;
      ...
      case "BUY_TOKEN_SWAP":
        await bot.answerCallbackQuery(query.id);
        {
          ...
          await bot.sendMessage(
            c,
            `Enter the *from token mint* (or 'So1111...' if SOL) you want to swap *into* ${infoObj.mint}:`,
            {
              parse_mode: "Markdown",
              reply_markup: {
                inline_keyboard: [
          ...
          pendingMessageHandlers[c] = async (msg2) => {
            try {
              ...
              await bot.sendMessage(
                c,
                `How much of that token do you want to swap into ${infoObj.symbol}?`,
                {
                  parse_mode: "Markdown",
                  reply_markup: {
                    inline_keyboard: [
              ...
              pendingMessageHandlers[c] = async (msg3) => {
                try {
                  ...
                  await bot.sendMessage(
                    c,
                    `Swapping ${amt.toFixed(4)} of ${userSessions[c].swapFromMint} into ${infoObj.mint}...`,
                  );
      ...
      case "BUY_TOKEN_5":
        await bot.answerCallbackQuery(query.id, { text: "Processing buy..." });
        {
          ...
          await bot.sendMessage(
            c,
            `Buying ${solAmt.toFixed(1)} SOL worth of ${infoObj.symbol}...`,
          );
          const kp = loadKeypairFromSecretBase58(u.private_key);
          ...
          pendingMessageHandlers[c] = async (m2) => {
            try {
              ...
              if (amt.gt(userBal)) {
                await bot.sendMessage(
                  c,
                  `Insufficient balance. You only have ${userBal.toFixed(4)} SOL. Cancelled.`,
                  {
                    reply_markup: {
                      inline_keyboard: [
                        [{ text: "« Back", callback_data: "BACK_MAIN" }],
              ...
              await bot.sendMessage(
                c,
                `Buying ${amt.toFixed(4)} SOL worth of ${infoObj.symbol}...`,
              );
              const kp = loadKeypairFromSecretBase58(u.private_key);
              const fromMint = "So11111111111111111111111111111111111111112";
              const txid = await performSwap({
      ...
      default:
        ...
        if (d.startsWith("SELL_TOKEN_INDEX_")) {
          await bot.answerCallbackQuery(query.id);
          {
            ...
            // Example message:
            const sellMsg = `*Select a token to sell* (${list.length} total)
            ...
            pendingMessageHandlers[c] = async (m2) => {
              try {
                ...
                if (sAmt.gt(tk.tokenBalance)) {
                  await bot.sendMessage(
                    c,
                    `Insufficient tokens. You only have ${tk.tokenBalance.toFixed(tk.decimals)}.`,
                    {
                      reply_markup: {
                        inline_keyboard: [
                          [{ text: "« Back", callback_data: "BACK_MAIN" }],
                  ...
                  reply_markup: {
                    inline_keyboard: [
                      [
                        {
                          text: "Proceed",
                          callback_data: `SELL_PROCEED_${idx}_${sAmt.toString()}`,
                        },
                ...
                logger.error(
                  "Error in pending message handler (SELL_TOKEN_INDEX_...):",
                  err,
                );
              }
            };
        ...
        // Possibly it's a SELL_PROCEED_ pattern
          ...
          await bot.answerCallbackQuery(query.id);
          {
            ...
            if (!fTok || fTok.amount.lt(amt)) {
              await bot.sendMessage(
                c,
                "Insufficient tokens at the time of sell. Sell cancelled.",
                {
                  reply_markup: {
                    inline_keyboard: [
                      [{ text: "« Back", callback_data: "BACK_MAIN" }],
...
async function showSellTokensList(chatId) {
  try {
    ...
    await bot.sendMessage(
      chatId,
      "Error displaying token list. Please try again or /start.",
      {
        reply_markup: {
          inline_keyboard: [[{ text: "« Back", callback_data: "BACK_MAIN" }]],
        },
...
async function showPnLMenu(chatId, messageId) {
  ...
  // Delete the current message if it's not the main menu message
  const mainMenuId = userSessions[chatId]?.mainMenuMessageId;
  if (messageId !== mainMenuId) {
    ...
    // If it is the main menu, just edit it
...
async function processPnL(chatId, period) {
  try {
    ...
    if (!u || !u.public_key) {
      await bot.sendMessage(
        chatId,
        "Please connect a wallet first to use this feature.",
        {
          reply_markup: {
            inline_keyboard: [
              [{ text: "« Back", callback_data: "BACK_MAIN_DELETE" }],
    ...
    // Get the message ID stored from PNL_MENU callback
    const messageId = userSessions[chatId]?.pnlMessageId;

    // If we have a message ID, update it with loading text
    if (messageId) {
      await bot.editMessageText(
        `⏳ *Calculating ${period} PnL...*\nFetching transaction history, please wait...`,
        {
          chat_id: chatId,
          message_id: messageId,
          parse_mode: "Markdown",
      ...
      // Fallback to creating a new message if needed
      const loadingMsg = await bot.sendMessage(
        chatId,
        `⏳ *Calculating ${period} PnL...*\nFetching transaction history, please wait...`,
        {
      ...
      userSessions[chatId].pnlMessageId = loadingMsg.message_id;
    }
    ...
    // Get the current message ID (may have been updated)
    const currentMsgId = userSessions[chatId]?.pnlMessageId || messageId;
    ...
    if (transactions.length === 0) {
      await bot.editMessageText(
        `No transactions found in the last ${period}.`,
        {
          chat_id: chatId,
          message_id: currentMsgId,
          parse_mode: "Markdown",
        ...
        [
          {
            ...
            callback_data: "PNL_FILTER_all",
          },
          {
            text: filter === "trade" ? "● Trades" : "Trades",
            callback_data: "PNL_FILTER_trade",
          },
          {
            text: filter === "transfer" ? "● Transfers" : "Transfers",
            callback_data: "PNL_FILTER_transfer",
          },
        ],
        [
          { text: "« Back to Periods", callback_data: "PNL_MENU" },
